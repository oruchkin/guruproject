...
class New_shop(APIView):
    ...
    def get(self, request): 
        ...
        if city and street and open == "0":
            nigth_shit_queryset = Shop.objects.filter(Q(time_open__gt=F('time_closed')), Q(time_closed__gt=time_now),
                                                      Q(city=city), Q(street=street))
            ...
            nigth_shit_queryset = Shop.objects.filter(Q(time_open__gt=F('time_closed')), Q(time_closed__gt=time_now),Q(city=city))
            day_shift_queryset = Shop.objects.filter(Q(time_open__lt=F('time_closed')), 
                                                     Q(Q(time_open__gt=time_now) , Q(time_closed__gt=time_now))| 
                                                     Q(Q(time_open__lt=time_now) , Q(time_closed__lt=time_now)),
                                                     Q(city=city))
            ...
            nigth_shit_queryset = Shop.objects.filter(Q(time_open__gt=F('time_closed')), Q(time_closed__gt=time_now), Q(street=street))
            day_shift_queryset = Shop.objects.filter(Q(time_open__lt=F('time_closed')), 
                                                     Q(Q(time_open__gt=time_now) , Q(time_closed__gt=time_now))| 
                                                     Q(Q(time_open__lt=time_now) , Q(time_closed__lt=time_now)),
                                                     Q(street=street))
